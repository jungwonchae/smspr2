...
public class DefaultController {
    ...
    public void download(@PathVariable("file_name") String file_name, @RequestParam Map<String, Object> map, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        String mimeType= URLConnection.guessContentTypeFromName(file_name);		//파일의 mime타입을 확인
        ...
        InputStream inputStream = new BufferedInputStream(new FileInputStream(file));	//inputstream 객체를 얻음
        FileCopyUtils.copy(inputStream, response.getOutputStream());		//inputstream으로 파일을 읽고 outputsream으로 파일을 씀
    }
}
